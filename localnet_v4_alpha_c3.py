...
import hashlib, secrets, sqlite3, zipfile, functools, shlex
...
def _write_block_acl():
    ...
    try:
        ...
        blocked_ips = [d["ip"] for d in devices
                       if d.get("blocked") and validate_ipv4(str(d.get("ip", "")))]
...
def restore_backup(zip_path: str) -> bool:
    ...
    Only accepts zip files that live inside BACKUP_DIR (path-traversal guard).
    Members are also checked for zip-slip before extraction."""
    try:
        p = Path(zip_path).resolve()
        # Reject any path that escapes the dedicated backup directory
        if not str(p).startswith(str(BACKUP_DIR.resolve())):
            log(f"Restore rejected — path outside backup dir: {zip_path}", "error")
            return False
        if not p.exists():
            log(f"Backup not found: {zip_path}", "error")
            return False
        safe_roots = ("/etc/bind", "/etc/nginx")
        with zipfile.ZipFile(str(p), 'r') as zf:
            for member in zf.infolist():
                # Normalise to catch ../.. tricks
                member_path = Path("/") / member.filename
                if not any(str(member_path).startswith(r) for r in safe_roots):
                    log(f"Restore rejected — unsafe member: {member.filename}", "error")
                    return False
            zf.extractall("/")
        log(f"✔ Restored from {p}", "success")
        return True
    except Exception as e:
...
_RE_HOSTNAME = re.compile(r'^[a-zA-Z0-9]([a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?$')
_RE_DOMAIN   = re.compile(r'^[a-zA-Z0-9]([a-zA-Z0-9\-]{0,62}[a-zA-Z0-9])?'
                           r'(\.[a-zA-Z0-9]([a-zA-Z0-9\-]{0,62}[a-zA-Z0-9])?)*$')
_RE_SRV_PART = re.compile(r'^_[a-zA-Z0-9\-]{1,40}$')

def validate_ipv4(ip: str) -> bool:
    """Return True only for a well-formed IPv4 address with octets 0-255."""
    m = _RE_IPV4.match(ip or "")
    if not m:
        return False
...
def validate_hostname(h: str) -> bool:
    ...
    return bool(_RE_HOSTNAME.match(h or ""))

def validate_domain_name(d: str) -> bool:
    """Simple domain / zone name — letters, digits, hyphens, dots."""
    return bool(_RE_DOMAIN.match(d or ""))

def validate_port(p) -> bool:
    """Port number 1-65535."""
    try:
        return 1 <= int(str(p)) <= 65535
    except Exception:
        return False

# ─── Python-Native Zone File Helpers (no shell required) ──────────────────────
def _update_serial(text: str) -> str:
    """Replace the 10-digit serial in a zone SOA with the current epoch second."""
    return re.sub(r'\d{10}(\s*;\s*Serial)', str(int(time.time())) + r'\1', text)

def zone_append_record(zone_file: str, record_line: str):
    """Append a DNS record to a zone file and bump the serial — pure Python."""
    p = Path(zone_file)
    content = p.read_text() if p.exists() else ""
    content = _update_serial(content)
    if not content.endswith("\n"):
        content += "\n"
    p.write_text(content + record_line + "\n")

def zone_remove_records(zone_file: str, prefix: str):
    """Remove all records whose line starts with `prefix` and bump the serial."""
    p = Path(zone_file)
    if not p.exists():
        return
    lines = p.read_text().splitlines(keepends=True)
    lines = [l for l in lines if not l.startswith(prefix)]
    p.write_text(_update_serial("".join(lines)))

def rev_zone_remove_ptr(rev_file: str, fqdn: str):
    """Remove PTR entries pointing to `fqdn` from a reverse zone file."""
    p = Path(rev_file)
    if not p.exists():
        return
    lines = p.read_text().splitlines(keepends=True)
    # Match only safe FQDN patterns in the PTR value field
    lines = [l for l in lines if f"PTR {fqdn}" not in l]
    p.write_text(_update_serial("".join(lines)))

def named_reload() -> tuple[bool, str]:
    """Run named-checkconf then reload named. Returns (success, output)."""
    try:
        r = subprocess.run(['named-checkconf'], capture_output=True, text=True, timeout=10)
        if r.returncode != 0:
            return False, (r.stdout + r.stderr).strip()
        r2 = subprocess.run(['systemctl', 'reload', 'named'],
...
def api_dns_install():
    ...
    # Validate every forwarder IP
    fwds = [f.strip() for f in fwds if validate_ipv4(f.strip())]
    if not fwds:
        return jsonify({"error": "No valid forwarder IPs provided"}), 400
...
"""
    ...
    sq_zone = shlex.quote(zone_file)
    sq_rev  = shlex.quote(rev_file)
    sq_dom  = shlex.quote(domain)
    sq_rev_zone = shlex.quote(rev_zone)
...
sed -i "s/SERIAL/$SERIAL/" {sq_zone}

echo "[5/8] Writing reverse zone..."
{b64w(rev_file, rzone)}
sed -i "s/SERIAL/$SERIAL/" {sq_rev}

echo "[6/8] Validating and starting BIND9..."
named-checkconf
named-checkzone {sq_dom} {sq_zone}
named-checkzone {sq_rev_zone} {sq_rev}
...
def api_add_a():
    ...
    def worker():
        create_backup("pre_add_a")
        try:
            zone_append_record(zone_file, f"{host}    IN  A   {ip}")
            if rev_file:
                zone_append_record(rev_file, f"{last}    IN  PTR {host}.{domain}.")
            ok, out = named_reload()
            if ok:
                log(f"✔ Added A record: {host}.{domain} → {ip}", "success")
            else:
                log(f"✘ named reload failed: {out}", "error")
        except Exception as e:
            log(f"Add A record error: {e}", "error")
    threading.Thread(target=worker, daemon=True).start()
    return jsonify({"ok": True})
...
def api_add_cname():
    ...
    if not validate_hostname(alias):
        return jsonify({"error": "Invalid alias hostname"}), 400
    if not validate_hostname(target):
        return jsonify({"error": "Invalid target hostname"}), 400

    zone_file = f"/etc/bind/db.{domain}"

    def worker():
        create_backup("pre_add_cname")
        try:
            zone_append_record(zone_file, f"{alias}    IN  CNAME {target}.{domain}.")
            ok, out = named_reload()
            if ok:
                log(f"✔ Added CNAME: {alias}.{domain} → {target}.{domain}", "success")
            else:
                log(f"✘ named reload failed: {out}", "error")
        except Exception as e:
            log(f"Add CNAME error: {e}", "error")
    threading.Thread(target=worker, daemon=True).start()
    return jsonify({"ok": True})
...
def api_remove_record(host):
    ...
    def worker():
        create_backup("pre_remove_record")
        try:
            zone_remove_records(zone_file, host)
            if rev_file:
                rev_zone_remove_ptr(rev_file, f"{host}.{domain}.")
            ok, out = named_reload()
            if ok:
                log(f"✔ Removed record: {host}", "success")
            else:
                log(f"✘ named reload failed: {out}", "error")
        except Exception as e:
            log(f"Remove record error: {e}", "error")
    threading.Thread(target=worker, daemon=True).start()
    return jsonify({"ok": True})
...
def api_dns_test():
    ...
    server = (data.get('server') or get_local_ip()).strip()

    # Validate both to prevent unexpected arguments being injected into dig's argv
    if not validate_ipv4(server):
        return jsonify({"result": "Invalid server IP", "ok": False}), 400
    # Allow hostnames / FQDNs with dots; reject anything with shell-special chars
    if not re.match(r'^[a-zA-Z0-9.\-_*]+$', name) or len(name) > 253:
        return jsonify({"result": "Invalid query name", "ok": False}), 400
...
def api_add_proxy():
    ...
    if not validate_domain_name(domain) or not domain:
        return jsonify({"error": "Invalid domain name"}), 400
    if not validate_ipv4(upstream_ip):
        return jsonify({"error": "Invalid upstream IP address"}), 400
    if not validate_port(port):
        return jsonify({"error": "Invalid port number"}), 400
    if template not in NGINX_CONFIGS:
        return jsonify({"error": "Unknown template"}), 400

    config_str = build_nginx_config(template, domain, upstream_ip, port, extras)

    def worker():
        create_backup("pre_add_proxy")
        try:
            avail_path = Path(f"/etc/nginx/sites-available/{domain}")
            enabl_path = Path(f"/etc/nginx/sites-enabled/{domain}")
            avail_path.write_text(config_str)
            # Symlink: remove stale link first if it exists
            if enabl_path.is_symlink() or enabl_path.exists():
                enabl_path.unlink()
            enabl_path.symlink_to(avail_path)
            # Validate nginx config and reload
            r = subprocess.run(['nginx', '-t'], capture_output=True, text=True, timeout=10)
            if r.returncode != 0:
                log(f"✘ nginx -t failed: {r.stderr.strip()}", "error")
                return
            subprocess.run(['systemctl', 'reload', 'nginx'], timeout=10)
            log(f"✔ Proxy: {domain} → {upstream_ip}:{port} [{template}]", "success")
        except Exception as e:
            log(f"Add proxy error: {e}", "error")
    threading.Thread(target=worker, daemon=True).start()
    return jsonify({"ok": True})
...
def api_remove_proxy(domain):
    ...
    def worker():
        create_backup("pre_remove_proxy")
        try:
            for sub in ('sites-enabled', 'sites-available'):
                p = Path(f"/etc/nginx/{sub}/{domain}")
                if p.exists() or p.is_symlink():
                    p.unlink()
            subprocess.run(['systemctl', 'reload', 'nginx'], timeout=10)
            log(f"✔ Removed proxy: {domain}", "success")
        except Exception as e:
            log(f"Remove proxy error: {e}", "error")
    threading.Thread(target=worker, daemon=True).start()
    return jsonify({"ok": True})
...
def api_mkcert_cert():
    ...
    # ── Input validation ──────────────────────────────────────────────────────
    # Allow an optional leading wildcard prefix (*.example.com)
    domain_base = raw_domain[2:] if raw_domain.startswith('*.') else raw_domain
    if not validate_domain_name(domain_base):
        return jsonify({"error": "Invalid domain name"}), 400
    if not validate_domain_name(tld):
        return jsonify({"error": "Invalid domain in server config"}), 400

    # Derive a filesystem-safe filename; validate it too
    file_name      = raw_domain.replace('*', 'wildcard')
    file_name_base = file_name[2:] if file_name.startswith('*.') else file_name
    if not validate_domain_name(file_name_base):
        return jsonify({"error": "Could not derive a safe cert filename"}), 400

    CERT_DIR.mkdir(parents=True, exist_ok=True)

    # ── Shell quoting — every user-derived value that enters the script ───────
    sq_caroot    = shlex.quote(str(CA_DIR))
    sq_cert_dir  = shlex.quote(str(CERT_DIR))
    sq_file_name = shlex.quote(file_name)
    sq_raw       = shlex.quote(raw_domain)
    sq_wildcard  = shlex.quote(f"*.{tld}")
    sq_tld       = shlex.quote(tld)

    # Only the mkcert invocation itself needs a shell script — nginx injection
    # is done entirely in Python below to avoid any further shell interpolation.
    mkcert_script = f"""#!/bin/bash
set -e
cd {sq_cert_dir}
CAROOT={sq_caroot} mkcert \\
    -cert-file {sq_file_name}.pem \\
    -key-file  {sq_file_name}-key.pem \\
    {sq_raw} {sq_wildcard} {sq_tld}
echo "mkcert finished."
"""

    def _nginx_ssl_inject():
        """Python-native SSL directive injection — zero shell interpolation."""
        cert_path = str(CERT_DIR / f"{file_name}.pem")
        key_path  = str(CERT_DIR / f"{file_name}-key.pem")
        # Check both the specific domain config and the zone TLD config
        for conf_name in (raw_domain, tld):
            conf_p = Path(f"/etc/nginx/sites-available/{conf_name}")
            if not conf_p.exists():
                continue
            content = conf_p.read_text()
            if 'ssl_certificate' in content:
                log(f"ℹ SSL already present in {conf_name} — skipping", "info")
                break
            ssl_block = (
                f"\n    listen 443 ssl;"
            ...
            # Insert immediately after the server_name directive
            content = re.sub(
                r'(server_name\s+[^;]+;)',
                r'\1' + ssl_block,
                content, count=1
            )
            conf_p.write_text(content)
            r = subprocess.run(['nginx', '-t'], capture_output=True, text=True, timeout=10)
            if r.returncode == 0:
                subprocess.run(['systemctl', 'reload', 'nginx'], timeout=10)
                log(f"✔ Nginx SSL auto-configured for {conf_name}", "success")
            else:
                log(f"✘ nginx -t failed after SSL inject: {r.stderr.strip()}", "error")
            break
        else:
            log("ℹ No matching Nginx config found — cert generated, manual config needed.", "info")

    def full_worker():
        create_backup("pre_mkcert_cert")
        script_path = "/tmp/_localnet_v4.sh"
        Path(script_path).write_text(mkcert_script)
        os.chmod(script_path, 0o755)
        log(f"▶ Generate cert: {raw_domain}", "info")
        try:
            proc = subprocess.Popen(
                ['bash', script_path],
                stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True
            ...
            proc.wait()
            if proc.returncode == 0:
                log(f"✔ Certificate ready: {CERT_DIR}/{file_name}.pem", "success")
                _nginx_ssl_inject()
            else:
                log(f"✘ mkcert failed (exit {proc.returncode})", "error")
        except Exception as e:
            log(f"mkcert error: {e}", "error")
...
def api_delete_cert(domain):
    ...
    # Validate the domain from the URL path
    domain_base = domain[2:] if domain.startswith('*.') else domain
    if not validate_domain_name(domain_base):
        return jsonify({"error": "Invalid domain name"}), 400
    ...
    def worker():
        create_backup("pre_delete_cert")
        try:
            # A. Remove certificate files with Python — no shell rm
            for p in (CERT_DIR / f"{file_name}.pem", CERT_DIR / f"{file_name}-key.pem"):
                if p.exists():
                    p.unlink()
                    log(f"Removed {p}", "info")

            # B. Python-native nginx SSL cleanup — no sed interpolation
            conf_p = Path(f"/etc/nginx/sites-available/{domain}")
            if conf_p.exists():
                lines   = conf_p.read_text().splitlines(keepends=True)
                cleaned = [
                    l for l in lines
                    if not re.match(r'\s*listen 443 ssl\s*;', l)
                    and not re.match(r'\s*ssl_', l)
                ]
                conf_p.write_text("".join(cleaned))
                r = subprocess.run(['nginx', '-t'], capture_output=True, text=True, timeout=10)
                if r.returncode == 0:
                    subprocess.run(['systemctl', 'reload', 'nginx'], timeout=10)
                    log(f"✔ Nginx cleaned and reloaded for {domain}", "success")
                else:
                    log(f"✘ nginx -t failed after SSL cleanup: {r.stderr.strip()}", "error")
            else:
                log(f"ℹ No Nginx config found for {domain} — cert files deleted.", "info")

            log(f"✔ Cert deleted: {domain}", "success")
        except Exception as e:
            log(f"Delete cert error: {e}", "error")
...
def api_add_txt():
    ...
    if not validate_hostname(host):
        return jsonify({"error": "Invalid hostname"}), 400
    # Strip any surrounding quotes, then re-quote safely
    value = value.strip('"\'')
    # Reject embedded quotes or newlines that could corrupt the zone file
    if '"' in value or '\n' in value or '\r' in value:
        return jsonify({"error": "Invalid TXT value"}), 400
    quoted_value = f'"{value}"'
    zone_file = f"/etc/bind/db.{domain}"

    def worker():
        create_backup("pre_add_txt")
        try:
            zone_append_record(zone_file, f"{host}    IN  TXT  {quoted_value}")
            ok, out = named_reload()
            if ok:
                log(f"✔ Added TXT: {host} = {quoted_value}", "success")
            else:
                log(f"✘ named reload failed: {out}", "error")
        except Exception as e:
            log(f"Add TXT error: {e}", "error")
    threading.Thread(target=worker, daemon=True).start()
    return jsonify({"ok": True})
...
def api_add_srv():
    ...
    cfg       = load_config()
    domain    = cfg['domain']
    service   = data.get('service', '').strip()   # e.g. _http
    proto     = data.get('proto', '').strip()     # e.g. _tcp
    priority  = data.get('priority', 10)
    weight    = data.get('weight', 5)
    port      = data.get('port', '')
    target    = data.get('target', '').strip()

    if not _RE_SRV_PART.match(service):
        return jsonify({"error": "Invalid SRV service name (must start with _)"}), 400
    if not _RE_SRV_PART.match(proto):
        return jsonify({"error": "Invalid SRV protocol (must start with _)"}), 400
    if not validate_port(port):
        return jsonify({"error": "Invalid port number"}), 400
    if not validate_hostname(target):
        return jsonify({"error": "Invalid target hostname"}), 400
    try:
        ...
        return jsonify({"error": "Priority and weight must be integers"}), 400

    zone_file = f"/etc/bind/db.{domain}"
    record    = f"{service}.{proto}  IN  SRV  {priority} {weight} {port} {target}.{domain}."

    def worker():
        create_backup("pre_add_srv")
        try:
            zone_append_record(zone_file, record)
            ok, out = named_reload()
            if ok:
                log(f"✔ Added SRV: {service}.{proto} → {target}:{port}", "success")
            else:
                log(f"✘ named reload failed: {out}", "error")
        except Exception as e:
            log(f"Add SRV error: {e}", "error")
    threading.Thread(target=worker, daemon=True).start()
    return jsonify({"ok": True})
...
def api_add_wildcard():
    ...
    def worker():
        create_backup("pre_add_wildcard")
        try:
            zone_append_record(zone_file, f"*    IN  A  {ip}")
            ok, out = named_reload()
            if ok:
                log(f"✔ Added wildcard: *.{domain} → {ip}", "success")
            else:
                log(f"✘ named reload failed: {out}", "error")
        except Exception as e:
            log(f"Add wildcard error: {e}", "error")
    threading.Thread(target=worker, daemon=True).start()
    return jsonify({"ok": True})
...
def api_devices_register(device_id):
    ...
    # Sanitise to a safe DNS label and validate it
    safe_host = re.sub(r'[^a-z0-9\-]', '-', hostname.lower()).strip('-')[:63]
    if not validate_hostname(safe_host):
        return jsonify({"error": "Invalid hostname"}), 400

    ip = device['ip']
    if not validate_ipv4(ip):
        return jsonify({"error": "Device has an invalid IP address in database"}), 500
    ...
    def worker():
        try:
            ...
            zone_remove_records(zone_file, safe_host)
            zone_append_record(zone_file, f"{safe_host}    IN  A   {ip}")
            if rev_file:
                zone_append_record(rev_file, f"{last}    IN  PTR {safe_host}.{domain}.")
            ok, out = named_reload()
            if ok:
                log(f"✔ Registered: {safe_host}.{domain} → {ip}", "success")
            else:
                log(f"✘ named reload failed: {out}", "error")
        except Exception as e:
            log(f"Register device error: {e}", "error")
    threading.Thread(target=worker, daemon=True).start()
...
def api_devices_unregister(device_id):
    ...
    # Re-validate the stored hostname before using it in any file operation
    if not validate_hostname(hostname):
        return jsonify({"error": "Stored hostname is invalid"}), 500
    ...
    def worker():
        try:
            zone_remove_records(zone_file, hostname)
            if rev_file:
                rev_zone_remove_ptr(rev_file, f"{hostname}.{domain}.")
            ok, out = named_reload()
            if ok:
                log(f"✔ DNS record removed: {hostname}.{domain}", "success")
            else:
                log(f"✘ named reload failed: {out}", "error")
        except Exception as e:
            log(f"Unregister device error: {e}", "error")
    threading.Thread(target=worker, daemon=True).start()
...
def api_dhcp_install():
    ...
    data    = request.json or {}
    cfg     = load_config()
    ip      = (data.get('server_ip') or cfg.get('server_ip') or get_local_ip()).strip()
    domain  = cfg['domain']
    parts   = ip.split('.')
    subnet  = f"{parts[0]}.{parts[1]}.{parts[2]}.0"
    netmask = "255.255.255.0"
    router  = data.get('router', f"{parts[0]}.{parts[1]}.{parts[2]}.1").strip()
    range_s = data.get('range_start', f"{parts[0]}.{parts[1]}.{parts[2]}.100").strip()
    range_e = data.get('range_end',   f"{parts[0]}.{parts[1]}.{parts[2]}.200").strip()
    lease   = data.get('lease_time', 3600)

    if not validate_ipv4(ip):
        return jsonify({"error": "Invalid server IP address"}), 400
    if not validate_ipv4(router):
        return jsonify({"error": "Invalid router IP address"}), 400
    if not validate_ipv4(range_s):
        return jsonify({"error": "Invalid range_start IP address"}), 400
    if not validate_ipv4(range_e):
        return jsonify({"error": "Invalid range_end IP address"}), 400
    try:
        lease = int(lease)
        if not (60 <= lease <= 604800):
            raise ValueError
    except (TypeError, ValueError):
        return jsonify({"error": "lease_time must be between 60 and 604800 seconds"}), 400
...
def api_certbot_cert():
    ...
    domain    = data.get('domain', '').strip()
    cfg       = load_config()
    server_ip = (cfg.get('server_ip') or get_local_ip()).strip()
    ...
    # Certbot accepts wildcard domains (*.example.com); validate the base
    domain_base = domain[2:] if domain.startswith('*.') else domain
    if not validate_domain_name(domain_base):
        return jsonify({"error": "Invalid domain name"}), 400

    if not validate_ipv4(server_ip):
        return jsonify({"error": "Server IP in config is invalid"}), 500

    # ── Shell quoting — nothing user-supplied enters the script unquoted ──────
    sq_email     = shlex.quote(email)
    sq_domain    = shlex.quote(domain)
    sq_server_ip = shlex.quote(server_ip)

    # The rfc2136.ini content is written via b64w() (base64 tunnel) so the
    # server_ip value never touches a shell command directly.
    rfc2136_conf = (
        f"dns_rfc2136_server = {server_ip}\n"
        f"dns_rfc2136_port = 53\n"
        f"dns_rfc2136_name = localnet-certbot-key\n"
        f"dns_rfc2136_secret = $KEY_SECRET\n"
        f"dns_rfc2136_algorithm = HMAC-SHA256\n"
    )
    # We can't b64w the whole file because $KEY_SECRET must be shell-expanded.
    # Instead write everything except the secret via Python and append only that
    # line from the shell. Build the static part:
    static_conf = (
        f"dns_rfc2136_server = {server_ip}\n"
        f"dns_rfc2136_port = 53\n"
        f"dns_rfc2136_name = localnet-certbot-key\n"
...
{b64w('/etc/localnet/certbot/rfc2136.ini', static_conf)}
printf 'dns_rfc2136_secret = %s\\n' "$KEY_SECRET" >> /etc/localnet/certbot/rfc2136.ini
